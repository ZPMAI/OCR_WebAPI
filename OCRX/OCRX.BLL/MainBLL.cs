...
namespace OCRX.BLL
{
    ...
    public class MainBLL
    {
        ...
        //残损记录
        public DataSet1.T_OCRX_DAMAGERow[] damage1;

        //残损记录
        public DataSet1.T_OCRX_DAMAGERow[] damage2;
        ...
        public CtosResult InitCntCtrl(DataSet1.T_OCRX_CNTRow row)
        {
            ...
            if (cntNo != "未识别")
            {
                ...
                //if (cntInfo.DS.Tables["CM_CONTAINERS"].Rows[0]["ISOCODE"].ToString() != row.CONTAINER_SHAPE || CheckIsoCode(row.CONTAINER_SHAPE) == null)
                //{
                //    cntCtrl.txtISO.ForeColor = Color.Red;
                //}
                ...
                cntCtrl.ckbIsDmg.Checked = cntInfo.DS.Tables["CM_CONTAINERS"].Rows[0]["ISDAMAGE"].ToString() == "Y"&&cctdbDAL.SelectCTOSDamageRecord99XX(decimal.Parse(cntInfo.DS.Tables["CM_CONTAINERS"].Rows[0]["CONTAINERID"].ToString())).Rows.Count>0;
                ...
                if (cntCtrl.ckbIsDmg.Checked)
                {
                    cntCtrl.btnDmg1.Text = string.IsNullOrEmpty(row.DmgMemo1)?"输入烂柜明细(1)": row.DmgMemo1;
                    cntCtrl.btnDmg2.Text = string.IsNullOrEmpty(row.DmgMemo2) ? "输入烂柜明细(2)" : row.DmgMemo2;
                    cntCtrl.btnDmg3.Text = string.IsNullOrEmpty(row.DmgMemo3) ? "输入烂柜明细(3)" : row.DmgMemo3;
                    cntCtrl.btnDmg4.Text = string.IsNullOrEmpty(row.DmgMemo4) ? "输入烂柜明细(4)" : row.DmgMemo4;
                    cntCtrl.btnDmg5.Text = string.IsNullOrEmpty(row.DmgMemo5) ? "输入烂柜明细(5)" : row.DmgMemo5;

                    cntCtrl.btnDmg1.Enabled = true;
                    cntCtrl.btnDmg2.Enabled = true;
                    cntCtrl.btnDmg3.Enabled = true;
                    cntCtrl.btnDmg4.Enabled = true;
                    cntCtrl.btnDmg5.Enabled = true;
                }
                else
                {
                    InitDamageCntrl();

                    row.DmgMemo1 = string.Empty;
                    row.DmgMemo2 = string.Empty;
                    row.DmgMemo3 = string.Empty;
                    row.DmgMemo4 = string.Empty;
                    row.DmgMemo5 = string.Empty;
                }
        ...
        public void NextStep(DataSet1.T_OCRX_CNTRow row)
        {
            ...
            if (IsBind())
            {
                ...
                if (cntCtrl.numBndl.Value > Convert.ToDecimal(cntCtrl.txtBndl1.Text))
                {
                    ...
                    InsertDamageInfo(damage1);

                    InitCntCtrlBind(row);
                }
                else
                {
                    InsertDamageInfo(damage2);
            ...
            else
            {
                ...
                if (cntCtrl.numMoves.Value > Convert.ToDecimal(cntCtrl.txtMove1.Text)&&row2!=null)
                {
        ...
        {
            if (Convert.ToDecimal(cntCtrl.txtMove1.Text) > 1)
            {

                if (row2.CSTATUS != Convert.ToDecimal(Config.CStatus.Fetched))
                {
        ...
        public void MarkExpcetion(int move)
        {
            if (row1.CSTATUS != Convert.ToDecimal(Config.CStatus.Fetched))
            {
                throw new Exception("记录已成功处理，不能转异常");
        ...
        /// 初始化残损信息
        /// </summary>
        public void InitDamageCntrl()
        {
            cntCtrl.btnDmg1.Text = "输入烂柜明细(1)";
            cntCtrl.btnDmg2.Text = "输入烂柜明细(2)";
            cntCtrl.btnDmg3.Text = "输入烂柜明细(3)";
            cntCtrl.btnDmg4.Text = "输入烂柜明细(4)";
            cntCtrl.btnDmg5.Text = "输入烂柜明细(5)";

            row1.DmgMemo1 = string.Empty;
            row1.DmgMemo2 = string.Empty;
            row1.DmgMemo3 = string.Empty;
            row1.DmgMemo4 = string.Empty;
            row1.DmgMemo5 = string.Empty;

            cntCtrl.btnDmg1.Enabled = false;
            cntCtrl.btnDmg2.Enabled = false;
            cntCtrl.btnDmg3.Enabled = false;
            cntCtrl.btnDmg4.Enabled = false;
            cntCtrl.btnDmg5.Enabled = false;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable SelectDamagePositionCode()
        {
            return OCRX.DAL.cctdbDAL.SelectDamagePositionCode();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable SelectDamageTypeCode()
        {
            return OCRX.DAL.cctdbDAL.SelectDamageTypeCode();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable SelectQCWorkRecord()
        {
            string containerid = row1.CONTAINERID == 0 ? GetContainerId(row1.CONTAINER_NO) : row1.CONTAINERID.ToString();
            return OCRX.DAL.cctdbDAL.SelectQCWorkRecord(row1.TRVAL_NO, containerid);
        }

        public string GetContainerId(string containerno)
        {
            DataTable dt= OCRX.DAL.cctdbDAL.GetContainerInfo(containerno);
            return dt.Rows.Count > 0 ? dt.Rows[0]["CONTAINERID"].ToString() : "0";
        }
        ...
        public void InsertDamageInfo(DataSet1.T_OCRX_DAMAGERow[] damageList)
        {
            if (damageList == null)
            {
                return;
            ...
            foreach(var d in damageList)
            {
                if (d != null)
                {
                    OCRX.DAL.cctdbDAL.InsertDamage(d);
                }
            }
        }
        ...
        /// <param name="row"></param>
        /// <returns></returns>
        public bool checkDamageMemo(DataSet1.T_OCRX_CNTRow row)
        {
            return string.IsNullOrEmpty(row.DmgMemo1) && string.IsNullOrEmpty(row.DmgMemo2) && string.IsNullOrEmpty(row.DmgMemo3) && string.IsNullOrEmpty(row.DmgMemo4) && string.IsNullOrEmpty(row.DmgMemo5);
        }
        ...
        /// <param name="row"></param>
        /// <returns></returns>
        public DataSet1.T_OCRX_DAMAGEDataTable SelectDmgRecord(string velaliase)
        {
            return OCRX.DAL.cctdbDAL.SelectDmgRecord(velaliase);
        }
        ...
        /// <param name="row"></param>
        /// <returns></returns>
        public DataSet1.T_OCRX_DAMAGEDataTable SelectDmgRecordEx(string velaliase)
        {
            return OCRX.DAL.cctdbDAL.SelectDmgRecordEx(velaliase);
        }
        ...
        /// <param name="row"></param>
        /// <returns></returns>
        public DataSet1.T_OCRX_CNTDataTable SelectCnt(decimal dockid)
        {
            return OCRX.DAL.cctdbDAL.SelectCnt(dockid);
        }
        ...
        public void Rollback()
        {
            ...
            row1.ISARCHIVED = "N";
            row1.CONTAINERID = 0;
            OCRX.DAL.cctdbDAL.RollbackCntStatus(row1);
            ...
            if (row2 != null)
            {
                ...
                row2.ISARCHIVED = "N";
                row2.CONTAINERID = 0;
                OCRX.DAL.cctdbDAL.RollbackCntStatus(row2);
            }
        }
        /// <summary>
        /// 残损回退
